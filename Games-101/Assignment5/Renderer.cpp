...
void Renderer::Render(const Scene& scene)
{
	...
	float half_height = std::tan(deg2rad(scene.fov * 0.5f)); //half height
	float imageAspectRatio = scene.width / (float)scene.height;
	float half_width = half_height * imageAspectRatio;
	...
	for (int j = 0; j < scene.height; ++j)
	{
		for (int i = 0; i < scene.width; ++i)
		{
			...
			float u = i / (float)scene.width;
			float v =  j / (float)scene.height;
			float x = -half_width + 2 * u * half_width;
			float y = (1-2 * v) * half_height;
			// TODO: Find the x and y positions of the current pixel to get the direction
			// vector that passes through it.
			// Also, don't forget to multiply both of them with the variable *scale*, and
			// x (horizontal) variable with the *imageAspectRatio*
			float length = 1.f / sqrt(x * x + y * y + 1);
			Vector3f dir = Vector3f(x * length, y * length, -length); // Don't forget to normalize this direction!
